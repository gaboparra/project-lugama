...
import { getRandomSong, validateAnswer, searchSongs, getGenres, getArtists } from '../api/songs'
...
export default function useGame(onUserUpdate) {
  ...
  const animate = () => {
    ...
    setCurrentTime(time)
    if (time >= limit) { stopAudio(); return }
  ...
  const loadSong = useCallback(async (g, d, a) => {
    ...
    setAttempt(1)
    try {
      const data = await getRandomSong(selectedGenre, selectedDifficulty, selectedArtist)
      setSong(data)
      if (audioRef.current) {
...
const changeArtist = useCallback((a) => {
  ...
  return {
    song, attempt, genre, genres, artist, artists, difficulty, feedback,
    revealedSong, guessDisabled, suggestions,
    isPlaying, currentTime, volume, gameOver,
    audioRef, TIME_LIMITS, MAX_ATTEMPTS,
    loadGenres, loadArtists, loadSong,
    changeGenre, changeDifficulty, changeArtist,
