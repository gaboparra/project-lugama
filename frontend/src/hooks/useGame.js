...
export default function useGame(onUserUpdate) {
  ...
  const attemptRef  = useRef(1)      // siempre sincronizado con attempt
  ...
  const loadSong = useCallback(async (selectedGenre = genre) => {
    stopAudio()
    setRevealed(null)
    ...
    setAttempt(1)
    try {
      const data = await getRandomSong(selectedGenre)
      setSong(data)
      if (audioRef.current) {
  ...
  return {
    song, attempt, genre, genres, feedback,
