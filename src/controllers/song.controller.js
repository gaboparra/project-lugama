...
export const addSong = async (req, res) => {
  try {
    ...
    const newSong = new Song({ title, artist, previewUrl, albumCover, difficulty });
...
// Obtener una canción aleatoria para el desafío
...
// Validar si el nombre que mandó el usuario es correcto
...
export const searchExternalSong = async (req, res) => {
    try {
        ...
        const songs = response.data.data.map(song => ({
            ...
            albumCover: song.album.cover_medium // Opcional, por si quieren usarlo en el front
        }));
...
export const searchSongsInDb = async (req, res) => {
    try {
        ...
        // Buscamos canciones que coincidan con el título o el artista
        // Usamos una expresión regular para que sea flexible e ignore mayúsculas
        const songs = await Song.find({
            $or: [
                { title: { $regex: q, $options: 'i' } },
                { artist: { $regex: q, $options: 'i' } }
            ]
        }).limit(50); // Solo sugerimos las primeras 5 coincidencias
...
export const seedDatabase = async (req, res) => {
    ...
    try {
        for (const artistQuery of artists) {
            ...
            const response = await axios.get(`https://api.deezer.com/search?q=${artistQuery}&limit=50`);
